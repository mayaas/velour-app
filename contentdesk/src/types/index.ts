...
export const PLATFORM_RULES: Record<Platform, { rules: string[]; disclosure: boolean; toneGuidance: string }> = {
  linkedin: {
    rules: ['Professional, thought-leadership tone only','Share genuine expertise — avoid promotional language','Product mention acceptable when professionally relevant','Engage with comments to build credibility'],
    disclosure: false,
    toneGuidance: 'Expert, concise, data-driven. Speak as an AI recruitment practitioner.',
  },
  reddit: {
    rules: ['Be a genuine community member, not a brand rep','Read subreddit rules — many ban self-promotion','Answer fully before any product mention','Disclose affiliation clearly if you mention Harmonai'],
    disclosure: true,
    toneGuidance: 'Conversational, humble, community-first. No buzzwords.',
  },
  quora: {
    rules: ['Answer must directly address the question','Disclose affiliation if mentioning Harmonai','Provide real value — avoid thin promotional answers'],
    disclosure: true,
    toneGuidance: 'Educational, expert. Lead with knowledge, not product.',
  },
  medium: {
    rules: ['Articles must be genuinely informative','Product mention must be contextually relevant','Label any sponsored content clearly'],
    disclosure: false,
    toneGuidance: 'Thoughtful, long-form. Prioritise insight over promotion.',
  },
  hackernews: {
    rules: ['Strictly no marketing language','Technically substantive answers only','Disclose if you built what you mention','HN strongly penalises promotional comments'],
    disclosure: true,
    toneGuidance: 'Technical, direct, humble. Show the work, not the pitch.',
  },
  devto: {
    rules: ['Developer-focused, practical content','Tutorials and how-tos are highly valued','Product mention must add value to the technical content'],
    disclosure: false,
