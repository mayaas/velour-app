...
export function checkCompliance(content: string, platform: Platform, includesLink: boolean, linkContext?: string): ComplianceResult {
  ...
  if (rules.disclosure && includesLink) {
    ...
    checks.push({ label: 'Affiliate disclosure required on this platform', status: hasDisclosure ? 'pass' : 'warn', note: hasDisclosure ? undefined : 'Add a disclosure e.g. "Full disclosure: I work at Harmonai."' } as const)
  }
  ...
  if (includesLink) {
    ...
    checks.push({ label: 'Product link is contextually justified', status: hasContext ? 'pass' : 'warn', note: hasContext ? undefined : 'Explain why Harmonai is specifically relevant to this question.' } as const)
  }
...
export async function generateDraft(opp: Opportunity, includeLink = false): Promise<{ title?: string; content: string }> {
...
function buildDraft(opp: Opportunity, includeLink: boolean): { title?: string; content: string } {
  if (opp.type === 'article' || opp.platform === 'medium') return buildArticle(includeLink)
  if (opp.platform === 'hackernews' || opp.platform === 'devto') return buildTechnical(includeLink)
  return buildQA(includeLink)
}

function buildQA(includeLink: boolean): { content: string } {
